...
class PredictedDataset(Dataset):
    ...
    def __init__(
        ...
        if "inta" in dataset.dataset_name:
            ...
            print(
                "Overwriting SS postprocessing parameters for INTA dataset to sep 0.5, min 0.5, max 5.0"
            )
    ...
    def set_probability_threshold(
        self, new_probability_threshold, adjusted_by_threshold=None, verbose=False
    ):
        ...
        if adjusted_by_threshold is not None:
            new_probability_threshold = (
                det_utils.transform_thr_for_adjusted_to_thr_for_predicted(
                    new_probability_threshold, adjusted_by_threshold
                )
            )
            (
                print("New threshold: %1.8f" % new_probability_threshold)
                if verbose
                else None
            )
    ...
    def _update_stamps(self):
        ...
        if self.task_mode == constants.N2_RECORD:
            # Keep only N2 stamps
            n2_pages_val = self.get_pages(pages_subset=constants.N2_RECORD)
        else:
            n2_pages_val = None

        stamps_list = self.postprocessor.proba2stamps_with_list(
            probabilities_list, pages_indices_subset_list=n2_pages_val, thr=0.5
        )  # thr is 0.5 because probas are adjusted
        ...
        if self.event_name == constants.KCOMPLEX:
            ...
            for k, sub_id in enumerate(self.all_ids):
                ...
                stamps = postprocessing.kcomplex_stamp_split(
                    signal, stamps, self.fs, signal_is_filtered=True
                )
        ...
        if "nsrr" in dataset_name:
            ...
            for k, sub_id in enumerate(self.all_ids):
                # Load signal
                sub_data = self.parent_dataset.read_subject_data(
                    sub_id, exclusion_of_pages=False
                )
                signal = sub_data["signal"]
                stamps = stamps_list[k]
                if stamps.size > 0:
                    stamps, no_peaks_found = postprocessing.spindle_amplitude_filtering(
                        signal, stamps, self.fs, max_amplitude
                    )
                    if no_peaks_found:
    ...
    def get_subject_stamps_probabilities(
        ...
        subject_proba = self.get_subject_probabilities(
            subject_id, return_adjusted=return_adjusted
        )
    ...
    def get_subset_stamps_probabilities(
        ...
        for sub_id in subject_ids:
            stamp_proba_list.append(
                self.get_subject_stamps_probabilities(
                    sub_id,
    ...
    def get_subject_probabilities(self, subject_id, return_adjusted=False):
        ...
        if return_adjusted:
            subject_probabilities = (
                det_utils.transform_predicted_proba_to_adjusted_proba(
    ...
    def get_subset_probabilities(self, subject_ids, return_adjusted=False):
        ...
        for sub_id in subject_ids:
            proba_list.append(
                self.get_subject_probabilities(sub_id, return_adjusted=return_adjusted)
            )
        return proba_list

    def get_probabilities(self, return_adjusted=False):
        return self.get_subset_probabilities(
            self.all_ids, return_adjusted=return_adjusted
        )
