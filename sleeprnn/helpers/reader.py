...
def read_predictions_crossval(ckpt_folder, parent_dataset, task_mode, verbose=False):
    print("Loading predictions") if verbose else None
    predictions_path = os.path.abspath(
        os.path.join(
            RESULTS_PATH, "predictions_%s" % parent_dataset.dataset_name, ckpt_folder
        )
    )
    if verbose:
...
def parse_prediction_files(ckpt_path):
    ...
    set_names = [
        pred_fname.split(".")[0].split("_")[-1] for pred_fname in available_preds
    ]
    path_dict = {
...
def read_prediction_with_seeds(
    ...
    for k in seed_id_list:
        ...
        ckpt_path = os.path.abspath(
            os.path.join(
                RESULTS_PATH, "predictions_%s" % dataset_name, ckpt_folder, "seed%d" % k
            )
        )
        this_dict = {}
...
def load_dataset(
    dataset_name, load_checkpoint=True, params=None, verbose=True, **kwargs
):
    # Load data
    name_to_class_map = {
